...
A license key file is a text file storing JSON encoded data. The data stored in a
license key file has a standardized format that can include customized data per a
third-party app's (app reading and verifying the license) needs. The Signature field
contains a public-private keypair signature of the other publically marshalled fields
in the license key file. The signature authenticates the data in the license key
file; if any data in the license key file is changed, or the signature is changed,
...
The process of creating a license key file and signing it is as follows:
 ...
 2. The File is marshalled to a byte slice.
 3. The bytes are hashed. This create's the license's fingerprint.
 4. The hash is signed via a previously defined private key.
 5. The generated signature is encoded into a textual format.
 6. The signature is added to the File's Signature field.
